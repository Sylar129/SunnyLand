...
Time::Time() {
  ...
  SPDLOG_TRACE("Time init。Last time: {}", last_time_);
}
...
void Time::SetTimeScale(float scale) {
  if (scale < 0.0) {
    SPDLOG_WARN("Time scale can't be negative. Clamping to 0.");
    scale = 0.0;
  }
  time_scale_ = scale;
}
...
void Time::SetTargetFps(int fps) {
  if (fps < 0) {
    SPDLOG_WARN("Target FPS can't be negative. Setting to 0 (unlimited).");
    target_fps_ = 0;
  } else {
    target_fps_ = fps;
  }
  ...
  if (target_fps_ > 0) {
    ...
    SPDLOG_INFO("Setting Target FPS: {} (Frame time: {:.6f}s)", target_fps_,
                target_frame_time_);
  } else {
    target_frame_time_ = 0.0;
    SPDLOG_INFO("Setting Target FPS: Unlimited");
  }
}
