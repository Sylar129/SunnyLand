...
GameState::GameState(SDL_Window* window, SDL_Renderer* renderer,
  ...
  ENGINE_ASSERT(window_ && renderer_, "window and renderer must not be null");
  ENGINE_TRACE("GameState initialized with state: {}",
               static_cast<int>(current_state_));
}

void GameState::SetState(State new_state) {
  if (current_state_ != new_state) {
    ENGINE_DEBUG("Changing game state from {} to {}",
                 static_cast<int>(current_state_), static_cast<int>(new_state));
    current_state_ = new_state;
  } else {
    ENGINE_DEBUG("SetState called with the same state: {}, no change made.",
...
void GameState::SetLogicalSize(const glm::vec2& new_size) {
  ...
  ENGINE_TRACE("SetLogicalSize: {}x{}", new_size.x, new_size.y);
}
