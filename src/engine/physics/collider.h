...
class Collider {
 protected:
  glm::vec2 aabb_size_ = {
      0.0f, 0.0f};  ///< @brief 覆盖Collider的最小包围盒的尺寸（宽度和高度）。

 public:
  virtual ~Collider() = default;
  virtual ColliderType getType() const = 0;  ///< @brief 获取碰撞器的类型。

  void setAABBSize(glm::vec2 size) {
    aabb_size_ = std::move(size);
  }  ///< @brief 设置最小包围盒的尺寸（宽度和高度）。
  const glm::vec2& getAABBSize() const {
    return aabb_size_;
...
class AABBCollider final : public Collider {
 private:
  glm::vec2 size_ = {0.0f,
                     0.0f};  ///< @brief 包围盒的尺寸（和aabb_size_相同）。
