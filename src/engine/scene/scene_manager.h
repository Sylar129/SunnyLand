...
class SceneManager final {
 ...
 public:
  ...
  void requestReplaceScene(
      std::unique_ptr<Scene>&& scene);  ///< @brief 请求替换当前场景。

  // getters
  Scene* getCurrentScene()
      const;  ///< @brief 获取当前活动场景（栈顶场景）的指针。
 ...
 private:
  void
  processPendingActions();  ///< @brief 处理挂起的场景操作（每轮更新最后调用）。
  // 直接切换场景
  void pushScene(
      std::unique_ptr<Scene>&&
          scene);   ///< @brief 将一个新场景压入栈顶，使其成为活动场景。
  void popScene();  ///< @brief 移除栈顶场景。
  void replaceScene(
      std::unique_ptr<Scene>&&
          scene);  ///< @brief 清理场景栈所有场景，将此场景设为栈顶场景。
};
