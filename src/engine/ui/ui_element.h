...
class UIElement {
 ...
 public:
  ...
  std::unique_ptr<UIElement> removeChild(
      UIElement*
          child_ptr);  ///< @brief 将指定子元素从列表中移除，并返回其智能指针
  ...
  // --- 辅助方法 ---
  engine::utils::Rect getBounds()
      const;  ///< @brief 获取(计算)元素的边界(屏幕坐标)
  glm::vec2 getScreenPosition() const;  ///< @brief 获取(计算)元素在屏幕上位置
  bool isPointInside(
      const glm::vec2& point) const;  ///< @brief 检查给定点是否在元素的边界内
