...
class HealthComponent final : public engine::component::Component {
 ...
 public:
  ...
  void heal(
      int heal_amount);  ///< @brief 治疗
                         ///< GameObject，增加当前生命值（不超过最大生命值）。
  ...
  void setCurrentHealth(
      int current_health);  ///< @brief 设置当前生命值 (确保不超过最大生命值)。
  void setMaxHealth(
      int max_health);  ///< @brief 设置最大生命值 (确保不小于 1)。
  void setInvincible(
      float duration);  ///< @brief 设置 GameObject 进入无敌状态，持续时间为
                        ///< duration 秒。
