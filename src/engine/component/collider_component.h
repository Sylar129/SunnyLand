...
class ColliderComponent final : public Component {
 ...
 private:
  TransformComponent* transform_ =
      nullptr;  ///< @brief 缓存的 TransformComponent 指针 (非拥有)

  std::unique_ptr<engine::physics::Collider>
      collider_;  ///< @brief 拥有的碰撞器对象。
  glm::vec2 offset_ = {
      0.0f,
      0.0f};  ///< @brief 碰撞器(最小包围盒的)左上角相对于变换原点的偏移量。
  engine::utils::Alignment alignment_ =
      engine::utils::Alignment::NONE;  ///< @brief 对齐方式。

  bool is_trigger_ =
      false;               ///< @brief 是否为触发器 (仅检测碰撞，不产生物理响应)
 ...
 public:
  ...
  engine::utils::Rect getWorldAABB()
      const;  ///< @brief 获取世界坐标系下的最小轴对齐包围盒（AABB）。
  ...
  void setAlignment(engine::utils::Alignment
                        anchor);  ///< @brief 设置新的对齐方式并重新计算偏移量。
