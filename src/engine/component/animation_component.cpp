...
void AnimationComponent::Update(float delta_time, engine::core::Context&) {
  ...
  if (!current_animation_->isLooping() &&
    ...
    is_playing_ = false;
    animation_timer_ =
        current_animation_->getTotalDuration();  // 将时间限制在结束点
    if (is_one_shot_removal_) {  // 如果 is_one_shot_removal_ 为
                                 // true，则删除整个 GameObject
...
bool AnimationComponent::isAnimationFinished() const {
  // 如果没有当前动画(说明从未调用过playAnimation)，或者当前动画是循环的，则返回
  // false
