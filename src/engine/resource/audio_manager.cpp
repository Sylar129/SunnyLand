...
AudioManager::AudioManager() {
  if (!MIX_Init()) {
    ENGINE_CRITICAL("AudioManager Error: Mix_Init failed: {}", SDL_GetError());
    return;
  }
  ...
  ENGINE_TRACE("Init AudioManager successfully");
}
...
MIX_Audio* AudioManager::GetSound(const std::string& file_path) {
  ...
  ENGINE_WARN("Sound '{}' not found. Try Loading.", file_path);
...
void AudioManager::UnloadSound(const std::string& file_path) {
  auto it = sounds_.find(file_path);
  if (it != sounds_.end()) {
    ENGINE_DEBUG("Unload sound: {}", file_path);
    sounds_.erase(it);
  } else {
    ENGINE_WARN("Try to unload non-existent sound: {}", file_path);
  }
}
