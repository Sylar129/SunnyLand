...
Renderer::Renderer(SDL_Renderer* sdl_renderer,
  ...
  ENGINE_TRACE("构造 Renderer...");
  if (!renderer_) {
    throw std::runtime_error(
        "Renderer 构造失败: 提供的 SDL_Renderer 指针为空。");
  }
  if (!resource_manager_) {
    // ResourceManager 是 drawSprite 所必需的
    throw std::runtime_error(
        "Renderer 构造失败: 提供的 ResourceManager 指针为空。");
  }
  setDrawColor(0, 0, 0, 255);
  ENGINE_TRACE("Renderer 构造成功。");
}
...
void Renderer::drawSprite(const Camera& camera, const Sprite& sprite,
  ...
  if (!isRectInViewport(camera,
                        dest_rect)) {  // 视口裁剪：如果精灵超出视口，则不绘制
    ENGINE_INFO("精灵超出视口范围，ID: {}", sprite.getTextureId());
    return;
  }
  ...
  if (!SDL_RenderTextureRotated(
    ...
    ENGINE_ERROR("渲染旋转纹理失败（ID: {}）：{}", sprite.getTextureId(),
                  SDL_GetError());
  }
}
...
void Renderer::drawParallax(const Camera& camera, const Sprite& sprite,
  ...
  if (repeat.x) {
    ...
    stop.x = glm::min(
        position_screen.x + scaled_tex_w,
        viewport_size.x);  // 结束点是一个纹理宽度之后，但不超过视口宽度
  }
  if (repeat.y) {
    ...
    stop.y = glm::min(
        position_screen.y + scaled_tex_h,
        viewport_size.y);  // 结束点是一个纹理高度之后，但不超过视口高度
  }
  ...
  for (float y = start.y; y < stop.y; y += scaled_tex_h) {
    for (float x = start.x; x < stop.x; x += scaled_tex_w) {
      ...
      if (!SDL_RenderTexture(renderer_, texture, nullptr, &dest_rect)) {
        ENGINE_ERROR("渲染视差纹理失败（ID: {}）：{}", sprite.getTextureId(),
                      SDL_GetError());
        return;
      }
    }
...
void Renderer::drawUISprite(const Sprite& sprite, const glm::vec2& position,
  ...
  if (!SDL_RenderTextureRotated(
          ...
          sprite.isFlipped() ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE)) {
    ENGINE_ERROR("渲染 UI Sprite 失败 (ID: {}): {}", sprite.getTextureId(),
                  SDL_GetError());
  }
}
...
void Renderer::setDrawColor(Uint8 r, Uint8 g, Uint8 b, Uint8 a) {
  if (!SDL_SetRenderDrawColor(renderer_, r, g, b, a)) {
    ENGINE_ERROR("设置渲染绘制颜色失败：{}", SDL_GetError());
  }
}
...
void Renderer::clearScreen() {
  if (!SDL_RenderClear(renderer_)) {
    ENGINE_ERROR("清除渲染器失败：{}", SDL_GetError());
  }
}
...
std::optional<SDL_FRect> Renderer::getSpriteSrcRect(const Sprite& sprite) {
  ...
  if (src_rect.has_value()) {  // 如果Sprite中存在指定rect，则判断尺寸是否有效
    if (src_rect.value().w <= 0 || src_rect.value().h <= 0) {
      ENGINE_ERROR("源矩形尺寸无效，ID: {}", sprite.getTextureId());
      return std::nullopt;
    }
    return src_rect;
    ...
    if (!SDL_GetTextureSize(texture, &result.w, &result.h)) {
      ENGINE_ERROR("无法获取纹理尺寸，ID: {}", sprite.getTextureId());
      return std::nullopt;
    }
    return result;
  }
...
bool Renderer::isRectInViewport(const Camera& camera, const SDL_FRect& rect) {
  ...
  return rect.x + rect.w >= 0 &&
         rect.x <= viewport_size.x &&  // 相当于 AABB碰撞检测
